...
function getFileNameWithNgrxExtension(path, type) {
...
function activate(context) {
    ...
    console.log('Congratulations, your extension "angular-file-changer" is now active!');
    // The command has been defined in the package.json file
    // Now provide the implementation of the command with  registerCommand
    // The commandId parameter must match the command field in package.json
    let cmdswitchHTML = vscode.commands.registerCommand("extension.switchHTML", () => {
        ...
        xOpenTextDocument(targetFile, isSplit ? vscode.ViewColumn.Two : editor.viewColumn).then(() => { }, err => {
    ...
    let cmdswitchTypeScript = vscode.commands.registerCommand("extension.switchTypeScript", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
    ...
    let cmdswitchCSS = vscode.commands.registerCommand("extension.switchCSS", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
    ...
    let cmdswitchTest = vscode.commands.registerCommand("extension.switchTest", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
    ...
    let cmdSwitchAction = vscode.commands.registerCommand("extension.switchAction", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
    ...
    let cmdSwitchEffect = vscode.commands.registerCommand("extension.switchEffect", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
    ...
    let cmdSwitchReducer = vscode.commands.registerCommand("extension.switchReducer", () => {
        if (!vscode.workspace) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
